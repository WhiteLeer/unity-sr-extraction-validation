Shader "Custom/NPR-3/ZZZURP"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // ZZZ profile: G=AO, R=SpecMask, A=MaterialId (B reserved for edge accent)
                float ao = mapSample.g;
                float specMask = mapSample.r;
            ...
            half4 Frag(Varyings IN) : SV_Target
            {
                ...
                float edgeAccent = saturate(packedMap.b);
                ...
                float rimBoost = lerp(1.0, 1.35, edgeAccent);
                float rim = pow(1.0 - saturate(dot(normalWS, viewDirWS)), _RimPower) * _RimStrength * rimBoost;
