Shader "Custom/NPR-3/CharacterAdvancedURP"
{
    Properties
    {
        ...
        [KeywordEnum(GenshinLike, HSRLike)] _PackedMapRule("Packed Map Rule", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float isHSR = step(0.5, _PackedMapRule);
                float ao = lerp(mapSample.g, mapSample.r, isHSR);
                float specMaskGenshin = max(mapSample.r, mapSample.b);
                float specMaskHSR = mapSample.b;
                float specMask = lerp(specMaskGenshin, specMaskHSR, isHSR);
            ...
            half4 Frag(Varyings IN) : SV_Target
            {
                ...
                float faceSoft = 0.08;
                float faceLightResponse = smoothstep(faceShadowMask - faceSoft, faceShadowMask + faceSoft, ndotlRaw);
                shadow *= lerp(1.0, faceLightResponse, faceShadowBlend);
        ...
        Pass
        {
            ...
            Varyings VertOutline(Attributes IN)
            {
                ...
                float2 offset = normalize(normalVS.xy) * (_OutlineWidth * 0.004 * distScale * silhouetteScale * widthMapScale);
